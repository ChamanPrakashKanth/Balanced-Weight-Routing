"""
Experiment 05: Verified Residual Routing (VRR: $D \to M_1 \to R_1 \to M_2 \to R_2 \dots$).
Escalates only the extracted residual subtask and failure deltas, minimizing context token overhead.
"""
...
from bwr.router import VerifiedResidualRouter
...
def run_exp05(
    ...
    router = VerifiedResidualRouter(profiles)
    orchestrator = Orchestrator(profiles=profiles, use_mock=use_mock, seed=seed)

    states = orchestrator.run_benchmark(tasks=tasks, router=router, experiment_name="exp05_verified_residual_routing")
    summary = MetricsAggregator.summarize_states(states, baseline_states=baseline_states)

    console.print("\n[bold blue]=== EXP05: Verified Residual Routing (VRR) ===[/bold blue]")
    table_data = [[
        "VRR (Residual)",
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="EXP05: Verified Residual Routing")
    ...
    run_exp05(use_mock=not args.live, max_tasks=args.tasks, seed=args.seed)
